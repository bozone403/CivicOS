...
export function registerLegalRoutes(app: Express) {
  ...
  app.get('/api/legal/acts', async (req: Request, res: Response) => {
    try {
      ...
      let query = db.select().from(legalActs) as any;
      ...
      if (search) {
        conditions.push(
          or(
            like(legalActs.title, `%${search}%`),
            like(legalActs.summary, `%${search}%`)
          )
        );
      }
  ...
  app.get('/api/legal/criminal-code', async (req: Request, res: Response) => {
    try {
      ...
      let query = db.select().from(criminalCodeSections) as any;
      const conditions: any[] = [];

      if (search) {
        conditions.push(
          or(
            like(criminalCodeSections.title, `%${search}%`),
            like(criminalCodeSections.content, `%${search}%`),
            like(criminalCodeSections.sectionNumber, `%${search}%`)
          )
        );
      }
      if (category) {
        conditions.push(eq(criminalCodeSections.category, category as string));
      }
  ...
  // Get Charter rights
  app.get('/api/legal/charter-rights', async (req: Request, res: Response) => {
    try {
    ...
    try {
      ...
      if (!type || type === 'acts') {
        const acts = await db
          ...
          .where(
            or(
              like(legalActs.title, `%${q}%`),
              like(legalActs.summary, `%${q}%`)
            )
          )
          .limit(10);
      ...
      if (!type || type === 'criminal') {
        const criminal = await db
          ...
          .where(
            or(
              like(criminalCodeSections.title, `%${q}%`),
              like(criminalCodeSections.content, `%${q}%`)
            )
          )
          .limit(10);
      ...
      if (!type || type === 'charter') {
        const charter = await db
          ...
          .where(
            or(
              like(charterRights.title, `%${q}%`),
              like(charterRights.description, `%${q}%`)
            )
          )
          .limit(10);
