...
export function registerVotingRoutes(app: Express) {
    ...
    try {
      ...
      if (userId) {
        const vote = await db
          ...
          .from(votes)
          .where(and(
            eq(votes.billId, parseInt(billId)),
            eq(votes.userId, userId)
          ))
          .limit(1);
      ...
      res.status(500).json({ message: "Failed to fetch bill voting data", error: (error as any)?.message || String(error) });
    }
  });

  // Get electoral candidates
  app.get('/api/voting/electoral/candidates', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch electoral candidates", error: (error as any)?.message || String(error) });
    }
  });

  // Get electoral results
  app.get('/api/voting/electoral/results', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch electoral results", error: (error as any)?.message || String(error) });
    }
  });
  ...
  app.get('/api/voting/electoral/user-votes', jwtAuth, async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch user electoral votes", error: (error as any)?.message || String(error) });
    }
  });
} 