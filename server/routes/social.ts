...
export function registerSocialRoutes(app: Express) {
  ...
  app.put('/api/social/posts/:id', jwtAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (String(existing[0].userId) !== String(currentUserId)) return res.status(403).json({ success: false, message: 'Not authorized to edit this post' });
      ...
      if (Object.keys(updates).length === 0) return res.json({ success: true, message: 'No changes' });
  ...
  app.delete('/api/social/posts/:id', jwtAuth, async (req: Request, res: Response) => {
    try {
      ...
      const existing = await db.select().from(socialPosts).where(eq(socialPosts.id, postId)).limit(1);
      if (existing.length === 0) return res.status(404).json({ success: false, message: 'Post not found' });
      if (String(existing[0].userId) !== String(currentUserId)) return res.status(403).json({ success: false, message: 'Not authorized to delete this post' });
  ...
  app.get('/api/social/comments/:targetType/:targetId', async (req: Request, res: Response) => {
    try {
      ...
      const commentsWithInteractions = await Promise.all(
        comments.map(async (comment) => {
          ...
          const dislikeCount = await db
            ...
            .where(and(
              eq(commentLikes.commentId, comment.id),
              eq(commentLikes.reaction, 'dislike')
            ));
  ...
  app.post('/api/social/comments', jwtAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any)?.id;
