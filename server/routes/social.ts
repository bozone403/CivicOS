...
export function registerSocialRoutes(app: Router) {
  ...
  app.get('/api/social/messages', jwtAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!otherUserId) {
       ...
       const messages = await db.select().from(userMessages).where(
         or(
  ...
  app.post('/api/social/follow', jwtAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Check if already following
      const existingFollow = await db.select().from(userFriends).where(
  ...
  app.get('/api/social/activity', jwtAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!userId) {
       ...
       ).orderBy(desc(userActivity.createdAt)).limit(20);
    ...
    try {
      ...
      if (!userId) {
        return res.status(401).json({ error: "Authentication required" });
             ...
             const bookmarks = await db.select().from(socialBookmarks).where(
    ...
    try {
      ...
      if (!userId) {
        return res.status(401).json({ error: "Authentication required" });
             ...
             const shares = await db.select().from(socialShares).where(
