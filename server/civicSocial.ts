...
router.get("/posts", jwtAuth, async (req: Request, res: Response) => {
  try {
    ...
    const postsWithDetails = await Promise.all(
      posts.map(async (post) => {
        ...
        const [isBookmarked] = await db
          ...
          .where(and(
            eq(socialBookmarks.postId, post.id),
...
router.post("/posts/:id/comments", jwtAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req.user as any).id || (req.user as any).sub;
    if (!userId) return res.status(401).json({ error: "Not authenticated" });
...
router.post("/posts/:id/share", jwtAuth, async (req: Request, res: Response) => {
  try {
    const originalPostId = parseInt(req.params.id, 10);
    const currentUserId = (req.user as any).id || (req.user as any).sub;
...
router.post("/posts/:id/bookmark", jwtAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req.user as any).id || (req.user as any).sub;
    if (!userId) return res.status(401).json({ error: "Not authenticated" });
