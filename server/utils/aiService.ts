...
class AiService {
  ...
  private extractPoliticianId(prompt: string): string {
    const lowerPrompt = prompt.toLowerCase();
    if (lowerPrompt.includes('carney')) return 'mark-carney';
    if (lowerPrompt.includes('trudeau')) return 'justin-trudeau';
    if (lowerPrompt.includes('poilievre')) return 'pierre-poilievre';
    if (lowerPrompt.includes('singh')) return 'jagmeet-singh';
    if (lowerPrompt.includes('blanchet')) return 'yves-francois-blanchet';
    if (lowerPrompt.includes('may')) return 'elizabeth-may';
    return 'mark-carney'; // Default to current PM
  }

  private extractBillId(prompt: string): string {
    const lowerPrompt = prompt.toLowerCase();
    if (lowerPrompt.includes('c-60')) return 'C-60';
    if (lowerPrompt.includes('c-56')) return 'C-56';
    if (lowerPrompt.includes('c-21')) return 'C-21';
    if (lowerPrompt.includes('c-61')) return 'C-61';
    return 'C-60'; // Default to climate finance bill
  }

  private extractFactCheckTopic(prompt: string): string {
    const lowerPrompt = prompt.toLowerCase();
    if (lowerPrompt.includes('carney')) return 'carney-transition';
    if (lowerPrompt.includes('housing')) return 'housing-crisis';
    if (lowerPrompt.includes('inflation')) return 'inflation-rates';
    if (lowerPrompt.includes('deficit')) return 'federal-deficit';
