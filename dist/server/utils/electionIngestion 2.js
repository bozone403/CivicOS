...
class ElectionIngestionService {
    ...
    async ingestProvincialElections() {
        try {
    ...
    async ingestMunicipalElections() {
        try {
    ...
    async upsertElection(electionData) {
        try {
            // Check if election already exists
            const existingElection = await db
                .select()
                .from(elections)
                .where(and(eq(elections.type, electionData.type), eq(elections.jurisdiction, electionData.jurisdiction), eq(elections.title, electionData.title)))
                .limit(1);
    ...
    async createSampleElectionsIfNeeded() {
        try {
    ...
    async getElectionsByLocation(location) {
        try {
            let electionsData;
            if (location) {
                ...
                electionsData = await db
                    .select()
                    .from(elections)
                    .where(or(ilike(elections.jurisdiction, `%${locationLower}%`), ilike(elections.title, `%${locationLower}%`)))
