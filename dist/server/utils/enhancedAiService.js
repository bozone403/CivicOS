...
class EnhancedAIService {
    ...
    async analyzeNewsBias(newsContent, source) {
        const prompt = `
    ...
    async generateLegislativeSummary(billContent, context) {
        const prompt = `
    ...
    parsePoliticalInsights(response) {
        // Simple parsing - in production, use more sophisticated parsing
        const insights = [];
        const sections = response.split('\n\n');
    ...
    parseCivicRecommendations(response) {
        const sections = response.split('\n\n');
        return {
    ...
    parseLegislativeSummary(response) {
        const sections = response.split('\n\n');
        return {
    ...
    parseCivicIntelligenceReport(response) {
        const sections = response.split('\n\n');
        return {
