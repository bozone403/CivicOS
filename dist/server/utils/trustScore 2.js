...
import { politicians, billRollcallRecords, campaignFinance, politicianTruthTracking } from '../../shared/schema.js';
import { eq } from 'drizzle-orm';
export async function computeTrustScore(politicianId) {
    // Voting consistency: favor clear, non-flip-flopping behavior
    const votes = await db.select().from(billRollcallRecords).where(eq(billRollcallRecords.memberId, (await getParliamentMemberId(politicianId))));
    ...
    const paired = votes.filter(v => v.decision === 'paired').length;
    ...
    const spendPenalty = fin && fin.length ? Math.min(20, (Number(fin[0].amount) || 0) / 10000) : 0;
    ...
    const truthScoreComponent = truth ? Math.max(0, 100 - Number(truth.truthScore || 0) * 20) : 10;
...
async function getParliamentMemberId(politicianId) {
    const [p] = await db.select().from(politicians).where(eq(politicians.id, politicianId));
    return p ? p.parliamentMemberId : null;
}
