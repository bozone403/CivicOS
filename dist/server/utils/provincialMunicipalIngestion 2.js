...
const BUILT_IN_MUNICIPAL_SOURCES = {
...
export function loadMunicipalCatalog() {
    try {
...
export function saveMunicipalCatalog(entries) {
    const dataUrl = new URL('../../data/municipal_sources.json', import.meta.url);
    const fsPath = dataUrl.pathname;
    try {
        mkdirSync(fsPath.replace(/\/municipal_sources\.json$/, ''), { recursive: true });
    }
    catch { }
    writeFileSync(fsPath, JSON.stringify(entries, null, 2), 'utf8');
}
export async function ingestProvincialIncumbents(provinceInput) {
    const provinces = provinceInput
        ? Object.keys(PROVINCIAL_SOURCES).filter(k => k.includes(provinceInput.toLowerCase()))
        : Object.keys(PROVINCIAL_SOURCES);
    let inserted = 0;
    let updated = 0;
            ...
            $('*').each((_i, el) => {
                ...
                if (name && likelyPersonName(name)) {
                    ...
                    db.insert(politicians).values(vals).onConflictDoNothing().returning().then(([row]) => {
                        ...
                        try {
                            await db.update(politicians).set(vals).where(politicians.name.eq(name));
                            updated++;
                        }
                        catch { }
                    });
...
export async function ingestMunicipalIncumbents(targets) {
...
function getProvincialTitle(key) {
    const map = {
...
function likelyPersonName(name) {
