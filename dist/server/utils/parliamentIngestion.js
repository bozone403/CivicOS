...
import { parliamentMembers, billRollcalls, billRollcallRecords } from '../../shared/schema.js';
import { eq } from 'drizzle-orm';
export async function ingestParliamentMembers() {
 ...
 */
export async function ingestBillRollcallsForCurrentSession(limit = 10) {
    let rollcalls = 0;
    let records = 0;
    const listUrl = process.env.OPENPARLIAMENT_VOTES_URL || `https://api.openparliament.ca/votes/?format=json&limit=${limit}`;
    try {
        ...
        const items = listJson?.results || listJson?.objects || listJson?.votes || listJson?.data || listJson || [];
        if (!Array.isArray(items))
            return { rollcalls, records };
        for (const item of items) {
            try {
                const voteId = String(item?.id || item?.pk || item?.vote_id || item?.number || item?.url?.split('/').filter(Boolean).pop() || '');
                const billNumber = String(item?.bill?.number || item?.bill_number || item?.related?.bill?.number || item?.label || item?.short_title ||
                    item?.title || '');
                const voteNumber = Number(item?.number || item?.vote_number || item?.division_number || item?.sequence || 0) || null;
                const parliament = Number(item?.parliament || item?.parliament_number || item?.session?.parliament_number || item?.session_parliament);
                ...
                if (billNumber) {
                    ...
                    if (existing.length > 0 && existing[0].voteNumber === voteNumber) {
                ...
                if (voterDetails) {
                    ...
                    if (Array.isArray(yes) && yes.length)
                        decisionBuckets.push({ decision: 'yes', members: yes });
                    if (Array.isArray(no) && no.length)
                        decisionBuckets.push({ decision: 'no', members: no });
                    if (Array.isArray(paired) && paired.length)
                        decisionBuckets.push({ decision: 'paired', members: paired });
                    if (Array.isArray(abstain) && abstain.length)
                        decisionBuckets.push({ decision: 'abstain', members: abstain });
                }
                if (decisionBuckets.length === 0) {
                    ...
                    if (Array.isArray(voters)) {
                        for (const v of voters) {
                            ...
                            const decision = String(v?.decision || v?.vote || v?.position || v?.result || v?.option || '').toLowerCase();
