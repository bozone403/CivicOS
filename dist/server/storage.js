import { users, bills, votes, politicians, politicianStatements, notifications, userNotificationPreferences, petitions, petitionSignatures, 
// userVotes,
// voteCounts,
userActivity,
// type Petition,
...
export class DatabaseStorage {
    ...
    async updateUserNotificationPreferences(userId, preferencesData) {
    ...
    async getVoteByUserAndItem(userId, itemId, itemType) {
        const [vote] = await db
            .select()
            .from(votes)
    ...
    async createPoliticianStatement(statement) {
        const [newStatement] = await db.insert(politicianStatements).values(statement).returning();
        return newStatement;
    }
    async getPoliticianStatements(politicianId) {
        return await db
            .select()
    ...
    async getPoliticianVotingRecord(politicianId) {
    ...
    async getPoliticianPolicyPositions(politicianId) {
    ...
    async getPoliticianPublicStatements(politicianId) {
        const statements = await db
            .select()
            .from(politicianStatements)
            .where(eq(politicianStatements.politicianId, politicianId))
    ...
    async getPoliticianFinancialDisclosures(politicianId) {
    ...
    calculateStatementImpact(content) {
    ...
    analyzeStatementSentiment(content) {
    ...
    async getPetitionSignature(petitionId, userId) {
    ...
    async signPetition(petitionId, userId, verificationId) {
        // Insert signature
        const [signature] = await db
    ...
    async checkPetitionTarget(petitionId) {
    ...
    async getAutoPetitionForBill(billId) {
        const [petition] = await db
            .select()
            .from(petitions)
            .where(and(eq(petitions.relatedBillId, billId), eq(petitions.autoCreated, true)));
        return petition;
    }
    ...
    async notifyVotersOfAutoPetition(billId, petitionId) {
        // Get all users who voted "no" on this bill
        const noVoters = await db
            .select({ userId: votes.userId })
            .from(votes)
            .where(and(eq(votes.itemId, billId), eq(votes.itemType, 'bill'), eq(votes.voteValue, -1)));
    ...
    async getUserCivicLedger(userId) {
        try {
            // Get user's votes
