...
export class VotingSystem {
     ...
     */
    async castVote(userId, itemId, optionId) {
        try {
     ...
     */
    async getActiveVotingItems(userId) {
        try {
      ...
      `);
            ...
            return {
                ...
                results: results.rows.map(row => ({
                    ...
                    optionText: options.find((opt) => opt.id === row.option_id)?.text || 'Unknown',
     ...
     */
    async createBillVote(billId, userId) {
        try {
     ...
     */
    async hasUserVoted(userId, itemId) {
        try {
    ...
    async getUserVotingHistory(userId) {
      ...
      `);
            return history.rows.map(row => {
                ...
                const selectedOption = options.find((opt) => opt.id === row.option_id);
                return {
                    id: row.id,
                    title: row.title,
                    type: row.type,
