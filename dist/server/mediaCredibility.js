...
export async function analyzeArticleCredibility(articleText, sourceName) {
...
function generateRecommendation(outlet, analysis) {
    if (!outlet)
        return "Unknown source - verify claims independently";
    const score = analysis.credibilityScore || outlet.credibilityScore;
    if (score >= 80)
        return "Highly reliable source - cross-reference for completeness";
    if (score >= 70)
        return "Generally reliable - check for bias in opinion sections";
    if (score >= 60)
        return "Moderate reliability - verify key claims with additional sources";
    return "Low reliability - requires significant fact-checking and verification";
}
export function getMediaOutletProfile(sourceName) {
    return canadianMediaOutlets.find(o => o.name.toLowerCase().includes(sourceName.toLowerCase()) ||
        sourceName.toLowerCase().includes(o.name.toLowerCase())) || null;
}
export function getMediaOutletFunding(sourceName) {
...
function analyzeConflictsOfInterest(outlet) {
    const conflicts = [];
