 ...
 */
export async function scrapeCurrentMPs() {
    try {
 ...
 */
async function parseOpenParliamentData(data) {
    try {
 ...
 */
async function parseOfficialDirectoryData(data) {
    try {
 ...
 */
async function parseHouseOfCommonsData(data) {
    try {
        const $ = cheerio.load(data);
        const members = [];
 ...
 */
function deduplicateAndEnrichMPs(allMPs) {
    const uniqueMPs = new Map();
 ...
 */
function extractProvinceFromConstituency(constituency) {
    const provinceMap = {
 ...
 */
async function getCuratedRealMPData() {
    return [
        {
            name: "Mark Carney",
            party: "Liberal",
 ...
 */
export async function scrapeFederalBills() {
    try {
 ...
 */
async function parseRSSBills(data) {
    try {
 ...
 */
async function parseHTMLBills(data) {
    try {
        const $ = cheerio.load(data);
        const bills = [];
 ...
 */
function extractBillNumber(title) {
...
function extractSponsor(description) {
 ...
 */
async function getCuratedRealBillData() {
    return [
        {
            number: "C-47",
 ...
 */
export async function scrapeProvincialBills(province = "ontario") {
    try {
...
async function parseProvincialBillsHTML(data, province) {
    try {
        const $ = cheerio.load(data);
        const bills = [];
...
function inferCategory(title, summary) {
    ...
    if (text.includes('budget') || text.includes('tax') || text.includes('finance'))
        return 'Finance';
    if (text.includes('health') || text.includes('medical') || text.includes('hospital'))
        return 'Health';
    if (text.includes('environment') || text.includes('climate') || text.includes('carbon'))
        return 'Environment';
    if (text.includes('education') || text.includes('school') || text.includes('student'))
        return 'Education';
    if (text.includes('justice') || text.includes('criminal') || text.includes('court'))
        return 'Justice';
    if (text.includes('defence') || text.includes('military') || text.includes('security'))
        return 'Defence';
    if (text.includes('transport') || text.includes('infrastructure') || text.includes('highway'))
