...
import { users, socialPosts, socialComments, socialLikes, userFriends, userMessages, notifications, userActivity, socialShares, socialBookmarks, userBlocks } from "../../shared/schema.js";
...
export function registerSocialRoutes(app) {
    ...
    app.get('/api/social/feed', jwtAuth, async (req, res) => {
        try {
            ...
            const posts = await db
                ...
                authorId: users.id,
                authorName: sql `COALESCE(${users.firstName} || ' ' || ${users.lastName}, ${users.username})`,
                ...
                sharesCount: sql `(SELECT COUNT(*) FROM ${socialShares} WHERE ${socialShares.postId} = ${socialPosts.id})`,
                isLiked: sql `EXISTS(SELECT 1 FROM ${socialLikes} WHERE ${socialLikes.postId} = ${socialPosts.id} AND ${socialLikes.userId} = ${userId})`,
            })
                .from(socialPosts)
            ...
            or(eq(socialPosts.visibility, 'public'), eq(socialPosts.userId, userId), and(eq(socialPosts.visibility, 'friends'), sql `EXISTS(SELECT 1 FROM ${userFriends} WHERE ((${userFriends.userId} = ${userId} AND ${userFriends.friendId} = ${socialPosts.userId}) OR (${userFriends.userId} = ${socialPosts.userId} AND ${userFriends.friendId} = ${userId})) AND ${userFriends.status} = 'accepted')`))))
    ...
    app.get('/api/social/posts', jwtAuth, async (req, res) => {
        try {
            ...
            if (authorId) {
                conditions.push(eq(socialPosts.userId, authorId));
            }
            const posts = await db
                .select({
                id: socialPosts.id,
    ...
    app.get('/api/social/comments', jwtAuth, async (req, res) => {
        try {
            ...
            const comments = await db
                ...
                authorName: sql `COALESCE(${users.firstName} || ' ' || ${users.lastName}, ${users.username})`,
                authorImage: users.profileImageUrl,
                likesCount: sql `(SELECT COUNT(*) FROM comment_likes WHERE comment_id = ${socialComments.id})`,
            })
                .from(socialComments)
    ...
    app.post('/api/social/posts/:postId/like', jwtAuth, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { postId } = req.params;
            // Check if already liked
            const existing = await db.select().from(socialLikes).where(and(eq(socialLikes.postId, Number(postId)), eq(socialLikes.userId, userId)));
            if (existing.length > 0) {
                // Unlike
                await db.delete(socialLikes).where(and(eq(socialLikes.postId, Number(postId)), eq(socialLikes.userId, userId)));
    ...
    app.post('/api/social/posts/:postId/comment', jwtAuth, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { postId } = req.params;
