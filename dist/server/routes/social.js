...
export function registerSocialRoutes(app) {
    ...
    app.post('/api/social/like', jwtAuth, async (req, res) => {
        try {
            const userId = req.user.sub;
            ...
            const existingLike = await db
                .select()
                .from(socialLikes)
                .where(and(eq(socialLikes.userId, userId), postId ? eq(socialLikes.postId, postId) : eq(socialLikes.commentId, commentId)))
                .limit(1);
    ...
    app.get('/api/social/friends-feed', jwtAuth, async (req, res) => {
        try {
            const userId = req.user.sub;
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 20;
