...
import { initializeGCKeyAuth, initializeBankingAuth, getAvailableAuthMethods, getProvincialAuthMethods } from "../canadianAuth";
// interface User {
//   id: string;
//   email: string;
//   isVerified?: boolean;
...
export function registerIdentityRoutes(app) {
    ...
    app.get('/api/identity/status', isAuthenticated, async (req, res) => {
        try {
    ...
    app.post('/api/identity/start-verification', isAuthenticated, async (req, res) => {
        try {
            // const userId = req.user.claims.sub;
    ...
    app.post('/api/identity/submit-step', isAuthenticated, async (req, res) => {
        try {
            // const userId = req.user.claims.sub;
    ...
    app.get('/api/admin/verification-queue', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
            const user = await storage.getUser(userId);
            ...
            const pendingVerifications = []; // Temporarily disabled - method doesn't exist
    ...
    app.post('/api/admin/approve-verification', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
            const user = await storage.getUser(userId);
            const isAdmin = user?.civicLevel === 'administrator';
    ...
    app.post('/api/admin/reject-verification', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
            const user = await storage.getUser(userId);
            const isAdmin = user?.civicLevel === 'administrator';
    ...
    app.post("/api/auth/gckey/init", async (req, res) => {
        try {
    ...
    app.post("/api/auth/banking/init", async (req, res) => {
