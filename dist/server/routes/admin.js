...
export function registerAdminRoutes(app) {
    ...
    app.get('/api/admin/moderation-dashboard', jwtAuth, requirePermission('view_analytics'), async (req, res) => {
        try {
            ...
            const recentPosts = await db
                .select()
                .from(socialPosts)
                .orderBy(socialPosts.createdAt.desc?.() || socialPosts.createdAt)
                .limit(limit)
                .offset(offset);
            const recentComments = await db
                .select()
                .from(socialComments)
                .orderBy(socialComments.createdAt.desc?.() || socialComments.createdAt)
                .limit(limit)
                .offset(offset);
            res.json({
                success: true,
