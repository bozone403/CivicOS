...
export function registerUserRoutes(app) {
    ...
    app.get('/api/users/search/public', async (req, res) => {
        try {
            ...
            const searchResults = await db
                ...
                .from(users)
                .where(or(ilike(users.firstName, `%${searchTerm}%`), ilike(users.lastName, `%${searchTerm}%`), ilike(users.city, `%${searchTerm}%`), ilike(users.province, `%${searchTerm}%`)))
    ...
    app.get('/api/users/search', jwtAuth, async (req, res) => {
        try {
            ...
            or(ilike(users.firstName, `%${searchTerm}%`), ilike(users.lastName, `%${searchTerm}%`), ilike(users.email, `%${searchTerm}%`), ilike(users.city, `%${searchTerm}%`), ilike(users.province, `%${searchTerm}%`))))
                .orderBy(desc(users.civicPoints))
                .limit(Number(limit))
                .offset(Number(offset));
