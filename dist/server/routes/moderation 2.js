...
export function registerModerationRoutes(app) {
    ...
    app.get('/api/moderation/comments/recent', jwtAuth, requirePermission('moderate_comments'), async (req, res) => {
        try {
    ...
    // Remove a comment
    app.delete('/api/moderation/comments/:id', jwtAuth, requirePermission('moderate_comments'), async (req, res) => {
        try {
    ...
    app.get('/api/moderation/posts/recent', jwtAuth, requirePermission('reject_content'), async (req, res) => {
        try {
            const { limit = '50', offset = '0' } = req.query;
            const rows = await db
                .select()
    ...
    // Remove a post
    app.delete('/api/moderation/posts/:id', jwtAuth, requirePermission('reject_content'), async (req, res) => {
        try {
            const id = Number(req.params.id);
            if (Number.isNaN(id))
                return res.status(400).json({ success: false, message: 'Invalid id' });
    ...
    app.get('/api/moderation/summary', jwtAuth, requirePermission('moderate_comments'), async (req, res) => {
        try {
