...
router.post('/bills/vote', async (req, res) => {
    try {
        ...
        const existingVote = await db.select().from(votes)
            .where(and(eq(votes.userId, userId), eq(votes.itemId, parseInt(billId)), eq(votes.itemType, 'bill')));
...
router.post('/electoral/vote', async (req, res) => {
    try {
        ...
        const existingVote = await db.select().from(electoralVotes)
            .where(and(eq(electoralVotes.userId, userId), eq(electoralVotes.candidateId, candidateId)));
        if (existingVote.length > 0) {
