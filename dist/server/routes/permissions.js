...
app.get("/api/permissions/me", jwtAuth, async (req, res) => {
    try {
        ...
        const userPerms = await PermissionService.getUserPermissions(userId, user[0].membershipType || 'citizen');
        res.json({
            success: true,
...
app.post("/api/permissions/check", jwtAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        const hasPermission = await PermissionService.hasPermission(userId, permissionName, user[0].membershipType || 'citizen');
        res.json({
            success: true,
            hasPermission,
            permissionName,
...
app.get("/api/permissions/all", jwtAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        const user = await db.select().from(users).where(eq(users.id, userId)).limit(1);
        if (user.length === 0) {
...
app.post("/api/permissions/grant", jwtAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        const success = await PermissionService.grantPermission(targetUserId, permissionName, userId, expiresAt ? new Date(expiresAt) : undefined, notes);
        if (success) {
            res.json({
                success: true,
...
app.post("/api/permissions/revoke", jwtAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
...
app.get("/api/permissions/user/:userId", jwtAuth, async (req, res) => {
    try {
        ...
        const userPerms = await PermissionService.getUserPermissions(targetUserId, targetUser[0].membershipType || 'citizen');
        res.json({
            success: true,
            user: targetUser[0],
            permissions: userPerms
