...
function isPoliticalContent(title, description) {
...
export function registerNewsRoutes(app) {
    ...
    app.get('/api/news', async (req, res) => {
        try {
            ...
            articles = articles.filter((article) => isPoliticalContent(article.title, article.summary || article.content || ''));
            // Apply filters
            if (source) {
                articles = articles.filter((article) => article.source === source);
            }
            if (bias) {
                articles = articles.filter((article) => article.bias === bias);
            }
            // Apply pagination
    ...
    app.get('/api/news/search', async (req, res) => {
        try {
            ...
            const conditions = [
                or(like(newsArticles.title, `%${q}%`), like(newsArticles.content, `%${q}%`), like(newsArticles.summary, `%${q}%`))
            ];
            if (source) {
            ...
            if (credibility) {
                conditions.push(gte(newsArticles.credibilityScore, parseInt(credibility)));
            }
    ...
    app.get('/api/news/:id/propaganda', async (req, res) => {
        try {
            const { id } = req.params;
            const [propaganda] = await db
                .select()
